...
int main(void)
{
    ...
    while (fgets(line, 256, stream)) {
        ...
        uint16_t y = 0;
        strcpy(field, line);
        sscanf(getfield(field, 6), "%" SCNu16, &y);
        strcpy(field, line);
        ...
        cass_cond(st1201_init_uint16(a, b, (unsigned)L, &st1201) == 0);
        cass_close(st1201.a, a);
        cass_close(st1201.b, b);
        cass_close(st1201.S_f, S_f);
        cass_close(st1201.S_r, S_r);
        cass_close(st1201.Z_o, Z_o);
        cass_equal_uint16(st1201_to_uint16(&st1201, x), y);
        cass_close(st1201_from_uint16(&st1201, y), xr);
    }
